...
public class Amf0RemotingServiceAuthenticationContextImpl extends
        RemotingServiceAuthenticationContextImpl {
    
    @Override
    protected final void doAuthenticate(final Message requestMessage) {
        final AmfObject credentials = (AmfObject) requestMessage
        ...
        if (credentials != null) {
            ...
            if (remotingServiceRealm.authenticate(credentialId,
                    credentialpassword)) {
                storage.savePrincipal(getUserPrincipalInRealm(credentialId));
            }
        }
    }
}