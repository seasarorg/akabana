...
public class FlashProjectHandler extends DefaultHandler{
	
    protected Pattern txtextensions = Pattern
    .compile(
            ".*\\.(|m?xml|as|(flex|actionScript)Properties|css|ppinfo)$",
    ...
    public void handle(ProjectBuilder builder, IProgressMonitor monitor) {
        try {
            ...
            if (Platform.getBundle(Constants.ID_FLEX_BUILDER_PLUGIN) != null) {
                ...
                project.getNature(Constants.ID_FLEX_BUILDER_ACTIONSCRIPTNATURE).configure(); // reconfigure
            }
        } catch (Exception e) {
            DoltengCore.log(e);
        }
